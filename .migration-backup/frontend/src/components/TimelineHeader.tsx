<<<<<<<< HEAD:artifacts/concierge/src/components/TimelineHeader.tsx
...
const TimelineHeader: React.FC = () => {
  ...
  return (
    <div style={{ background: '#F0F8FF', color: '#64748B', padding: '6px 10px', fontSize: 12, position: 'relative', borderBottom: '1px solid #DBEAFE' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <button onClick={() => setOpen((v) => !v)} style={{ marginLeft: 'auto', background: 'transparent', border: 'none', color: '#94A3B8', cursor: 'pointer' }}>{open ? '▴' : '▾'}</button>
      </div>

      {open && (
        <div style={{ position: 'absolute', right: 10, top: '100%', marginTop: 8, background: '#FFFFFF', border: '1px solid #DBEAFE', borderRadius: 8, padding: 12, width: 540, zIndex: 60, boxShadow: '0 4px 20px rgba(37,99,235,0.12)' }}>
          <div style={{ display: 'flex', gap: 12 }}>
            <div style={{ flex: '0 0 220px' }}>
              <img
                src={graphSrc}
                alt="timeline graph large"
                style={{ width: '100%', borderRadius: 6, border: '1px solid #DBEAFE' }}
                loading="lazy"
                decoding="async"
                onError={(e) => { (e.currentTarget as HTMLImageElement).src = placeholderPath; }}
              />
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#0F172A' }}>Timeline</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 240, overflowY: 'auto' }}>
                {tasks.map((t: any, idx: number) => (
                  <div key={t.task_id || idx} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <button onClick={() => { selectTimelineTask(t); setOpen(false); }} style={{ background: 'transparent', border: 'none', color: '#334155', textAlign: 'left', cursor: 'pointer', padding: '6px 8px', borderRadius: 6 }}>
                      <div style={{ fontWeight: 700, fontSize: 13 }}>{t.title || `Task ${idx + 1}`}</div>
                      <div style={{ fontSize: 12, color: '#64748B' }}>{(t.instructions || '').slice(0, 100)}</div>
                    </button>
                  </div>
                ))}
              </div>
          ...
          {selectedTaskMeta && (
            <div style={{ marginTop: 10, background: '#F0F8FF', padding: 8, borderRadius: 6, border: '1px solid #DBEAFE' }}>
              <pre style={{ whiteSpace: 'pre-wrap', margin: 0, color: '#334155', fontSize: 12 }}>{JSON.stringify(selectedTaskMeta, null, 2)}</pre>
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 8 }}>
                <button onClick={() => selectTimelineTask(null)} style={{ background: 'transparent', border: '1px solid #DBEAFE', borderRadius: 6, padding: '6px 10px', color: '#64748B', cursor: 'pointer' }}>Close</button>
              </div>
            </div>
          )}
        </div>
...
export default TimelineHeader;
>>>>>>>> f665b8188591020c7f82f8a93d3211e3cc2ffcb5:.migration-backup/frontend/src/components/TimelineHeader.tsx
