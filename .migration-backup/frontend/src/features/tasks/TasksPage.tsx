<<<<<<<< HEAD:artifacts/concierge/src/features/tasks/TasksPage.tsx
...
const TasksPage: React.FC = () => {
  ...
  return (
    <div className="page-content" style={{ maxWidth: 950, margin: '0 auto' }}>
      <ProcessingBanner />
      <h1 style={{ fontSize: 26, fontWeight: 800, margin: '0 0 8px', letterSpacing: '-0.01em', color: '#0F172A' }}>✅ Tasks</h1>
      <p style={{ fontSize: 14, color: '#475569', margin: '0 0 24px', lineHeight: 1.7 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 12 }}>
        {TASK_SUBNAV.map((item) => (
          <button
            ...
            style={{
              border: `1px solid ${activeTab === item.key ? '#2563EB' : '#DBEAFE'}`,
              background: activeTab === item.key ? '#2563EB' : '#F0F8FF',
      ...
      <div style={{ marginBottom: 28, color: '#64748B', fontSize: 13, lineHeight: 1.7 }}>
      ...
      <div style={{ display: 'grid', gap: 16, gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', marginBottom: 28 }}>
        {SAMPLE_LAYOUT_CARDS.map((card) => (
          <div key={card.title} style={{ borderRadius: 14, overflow: 'hidden', boxShadow: '0 4px 16px rgba(37,99,235,0.1)', border: '1px solid #DBEAFE', background: '#FFFFFF' }}>
            <img src={card.src} alt={card.title} style={{ width: '100%', height: 190, objectFit: 'cover', display: 'block' }} />
            <div style={{ padding: 16 }}>
              <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: '#0F172A' }}>{card.title}</h3>
              <p style={{ margin: '10px 0 0', color: '#475569', fontSize: 13, lineHeight: 1.65 }}>{card.caption}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {!loading && offline && (
        <div style={{ background: 'rgba(220,38,38,0.06)', border: '1px solid rgba(220,38,38,0.2)', borderRadius: 8, padding: '10px 16px', marginBottom: 24, fontSize: 13, color: '#B91C1C', display: 'flex', alignItems: 'center', gap: 8 }}>
          <span>⚠️</span>
      ...
      <div style={{ marginBottom: 32, background: '#F0F8FF', border: '1px solid #BFDBFE', borderRadius: 12, padding: '20px 22px' }}>
        <h2 style={{ fontSize: 15, fontWeight: 700, color: '#2563EB', margin: '0 0 6px' }}>🚀 Distributed Agent Job</h2>
        ...
        <form onSubmit={handleSubmitJob} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <input
            ...
            style={{ background: '#FFFFFF', border: '1px solid #BFDBFE', borderRadius: 7, padding: '9px 12px', color: '#0F172A', fontSize: 13, outline: 'none' }}
          />
          <input
            value={jobContext}
          ...
          />
          {submitError && (
            <div style={{ fontSize: 12, color: '#B91C1C', background: 'rgba(220,38,38,0.06)', border: '1px solid rgba(220,38,38,0.2)', borderRadius: 6, padding: '6px 10px' }}>{submitError}</div>
          )}
          <button
            type="submit"
            disabled={submitting || !jobGoal.trim()}
            style={{ alignSelf: 'flex-start', background: submitting || !jobGoal.trim() ? '#BFDBFE' : '#2563EB', color: '#fff', border: 'none', borderRadius: 7, padding: '9px 20px', fontSize: 13, fontWeight: 600, cursor: submitting || !jobGoal.trim() ? 'default' : 'pointer', transition: 'background 0.15s' }}
          >
        ...
        {activeJobs.length > 0 && (
          <div style={{ marginTop: 18 }}>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: '#94A3B8', marginBottom: 8 }}>Active / Recent Jobs</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {activeJobs.map((job) => {
                ...
                return (
                  <div key={job.id} style={{ background: '#FFFFFF', border: `1px solid ${(statusColor[st] ?? '#6b7280')}33`, borderRadius: 8, padding: '10px 14px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: job.statusObj.result || job.statusObj.error ? 6 : 0 }}>
                      ...
                      <span style={{ fontSize: 12, color: '#0F172A', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{job.label}</span>
                      <span style={{ padding: '2px 8px', borderRadius: 99, background: (statusColor[st] ?? '#6b7280') + '22', color: statusColor[st] ?? '#6b7280', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', flexShrink: 0 }}>{st}</span>
                    </div>
                    <div style={{ fontFamily: 'monospace', fontSize: 10, color: '#94A3B8' }}>{job.id}</div>
                    {job.statusObj.error && (
                      <div style={{ fontSize: 12, color: '#B91C1C', marginTop: 6, background: 'rgba(220,38,38,0.06)', borderRadius: 5, padding: '4px 8px' }}>{job.statusObj.error}</div>
                    )}
                    {job.statusObj.result ? (
                      <pre style={{ fontSize: 11, color: '#065F46', marginTop: 6, background: 'rgba(5,150,105,0.06)', border: '1px solid rgba(5,150,105,0.15)', borderRadius: 5, padding: '6px 8px', overflowX: 'auto', maxHeight: 120, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                        {typeof job.statusObj.result === 'string'
                          ? (job.statusObj.result as string)
      ...
      {!loading && tasks.length > 0 && (
        <div style={{ marginBottom: 32 }}>
          <h2 style={{ fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#94A3B8', margin: '0 0 10px', paddingBottom: 8, borderBottom: '1px solid #DBEAFE' }}>Recent tasks</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {tasks.slice(0, 8).map((t) => (
              <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 12, background: '#F8FAFF', border: '1px solid #DBEAFE', borderRadius: 7, padding: '8px 14px', fontSize: 13 }}>
                <span style={{ fontFamily: 'monospace', fontSize: 11, color: '#2563EB', minWidth: 100, maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.type}</span>
                <span style={{ padding: '2px 8px', borderRadius: 99, background: (statusColor[t.status] ?? '#6b7280') + '22', color: statusColor[t.status] ?? '#6b7280', fontSize: 11, fontWeight: 600 }}>{t.status}</span>
                <span style={{ fontFamily: 'monospace', fontSize: 10, color: '#94A3B8', marginLeft: 'auto' }}>{t.id.slice(0, 12)}…</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* task type reference */}
      <div style={{ marginBottom: 32 }}>
        <h2 style={{ fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#94A3B8', margin: '0 0 10px', paddingBottom: 8, borderBottom: '1px solid #DBEAFE' }}>Available task types</h2>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {TASK_TYPES.map(({ type, badge, payload, desc }) => (
            <div key={type} style={{ background: '#F8FAFF', border: '1px solid #DBEAFE', borderRadius: 8, padding: '12px 14px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                <span style={{ fontFamily: 'monospace', fontSize: 12, fontWeight: 700, color: badge }}>{type}</span>
              </div>
              <div style={{ fontSize: 12, color: '#475569', marginBottom: 6 }}>{desc}</div>
              <code style={{ fontSize: 11, color: '#1E40AF', background: '#EFF6FF', border: '1px solid #DBEAFE', padding: '4px 8px', borderRadius: 5, display: 'block', overflowX: 'auto', whiteSpace: 'pre' }}>payload: {payload}</code>
            </div>
          ))}
        </div>
      </div>
...
export default TasksPage;
>>>>>>>> f665b8188591020c7f82f8a93d3211e3cc2ffcb5:.migration-backup/frontend/src/features/tasks/TasksPage.tsx
