  ...
  return (
    <div style={{ background: '#111827', color: '#9ca3af', padding: '6px 10px', fontSize: 12, position: 'relative' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <img src={makeApiUrl(`/api/v1/concierge/timeline/graph?v=${encodeURIComponent((timelinePlan && (timelinePlan.updated_at || '')) || String(Date.now()))}`)} alt="timeline graph" style={{ height: 36, cursor: 'pointer' }} onClick={() => setOpen((v) => !v)} />
        ...
        <div style={{ position: 'absolute', right: 10, top: '100%', marginTop: 8, background: '#0b1220', border: '1px solid #263244', borderRadius: 8, padding: 12, width: 540, zIndex: 60 }}>
          <div style={{ display: 'flex', gap: 12 }}>
            <div style={{ flex: '0 0 220px' }}>
              <img src={makeApiUrl(`/api/v1/concierge/timeline/graph?v=${encodeURIComponent((timelinePlan && (timelinePlan.updated_at || '')) || String(Date.now()))}`)} alt="timeline graph large" style={{ width: '100%', borderRadius: 6 }} />
            </div>
