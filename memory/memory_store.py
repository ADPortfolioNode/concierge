...
class MemoryStore:
    ...
    async def async_init(self, timeout: int = 60, retry_interval: int = 2, progress_callback: Optional[callable] = None) -> None:
        """Asynchronously initialize the vector DB client and collection.

        This method should be called during the application's async startup
        lifespan to prevent blocking the main thread.
        """
        start_time = time.time()
        while time.time() - start_time < timeout:
            if progress_callback:
                progress_callback(f"Attempting to connect to {self._vector_db}...")
            try:
                if self._vector_db == "chroma":
                    if chromadb is None:
                        logger.warning("chromadb package is unavailable; skipping Chroma initialization")
                        if progress_callback:
                            progress_callback("Chroma package not found, skipping.")
                        break
                    ...
                    if _ONNX_AVAILABLE:
                        try:
                            chroma_path = os.getenv("CHROMA_PATH", os.path.join(os.getcwd(), "chroma_db"))
                            os.makedirs(chroma_path, exist_ok=True)
                            self._client = chromadb.PersistentClient(path=chroma_path)
                            logger.info("ChromaDB PersistentClient initialised at %s", chroma_path)
                        except Exception as exc:
                            logger.warning("ChromaDB PersistentClient failed; trying HTTP fallback at %s:%s (%s)", self._chroma_host, self._chroma_port, exc)
                            use_remote_chroma = True
                    else:
                        if self._chroma_host in ("localhost", "127.0.0.1") and self._chroma_port == os.getenv("PORT", "8000"):
                            logger.warning(
                            ...
                            if progress_callback:
                                progress_callback("ONNX unavailable, using in-memory fallback.")
                            break # Exit retry loop, fall back to in-memory
                        else:
                            logger.warning("onnxruntime not available; attempting remote Chroma HTTP client at %s:%s", self._chroma_host, self._chroma_port)
                            use_remote_chroma = True

                    if use_remote_chroma:
                        self._client = _init_chroma_http_client(self._chroma_host, self._chroma_port)
                        logger.info("ChromaDB HttpClient initialised at %s:%s", self._chroma_host, self._chroma_port)

                    if self._client is not None:
                        self._collection = _ensure_collection(self._client)
                        logger.info("ChromaDB collection '%s' ready.", self._collection_name)
                        if progress_callback:
                            progress_callback(f"ChromaDB collection '{self._collection_name}' is ready.")
                        return # Success, exit retry loop

                elif self._vector_db == "qdrant":
                    if QdrantClient is None:
                        logger.warning("Qdrant client not installed; skipping Qdrant initialization")
                        if progress_callback:
                            progress_callback("Qdrant package not found, skipping.")
                        break

                    try:
                        self._client = QdrantClient(host=self._qdrant_host, port=int(self._qdrant_port))
                    except Exception:
                        self._client = QdrantClient(url=f"http://{self._qdrant_host}:{self._qdrant_port}")
                    ...
                    if self._client is not None:
                        ...
                        logger.info("Connected to Qdrant at %s:%s", self._qdrant_host, self._qdrant_port)
                        self._is_qdrant = True
                        if progress_callback:
                            progress_callback(f"Qdrant connection to '{self._qdrant_host}:{self._qdrant_port}' is ready.")
                        return # Success, exit retry loop

            except Exception as e:
                logger.debug(f"Vector DB initialization failed: {e}. Retrying in {retry_interval}s...")
                if progress_callback:
                    progress_callback(f"Connection to {self._vector_db} failed, retrying...")
            await asyncio.sleep(retry_interval)

        logger.warning("Timed out connecting to vector DB. Falling back to in-memory store.")
        if progress_callback:
            progress_callback("Timed out connecting to vector DB. Using in-memory store.")
        self._client = None
        self._collection = None
